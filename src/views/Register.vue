...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        nickname: [
          { required: true, message: '起名都不会吗？', trgger: 'blur' },
          { min: 2, max: 16, message: '长度在2-16位字符之间', trgger: 'blur' },
        ],
        phone: [
        ...
        captcha: [
          { require: true, message: '验证码收不到?', trigger: 'blur' },
  ...
  methods: {
    ...
    _submit() {
      this.$refs.form.validate((valid) => {
        ...
        if (this.verifyData) {
          try {
            this.$api.reqister(this.formData).then((res) => {
              if (res.data.code === 200) {
                this.$message.success('总算注册成功啦!')
    ...
    // 发送验证码
    async _getCaptcha() {
      let res = await this.$api._getCaptcha(this.formData.phone)
    ...
    async _verifyCaptcha() {
      ...
      let res = await this.$api._verifyCaptcha(params)
