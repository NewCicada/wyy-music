    ...
    <div class="centent">
      ...
      >
        ...
        <span class="total-time">{{
          currentSong.transitionTime | formatPlayTime
        }}</span>
      </div>
      <div class="volnum-control">
        <i :class="isMutedIcon" @click="mutedClick"></i>
        <el-slider v-model="volumeSlider" @change="setVolume(volumeSlider)">
        </el-slider>
      </div>
  ...
  methods: {
    ...
    ended() {
      if (this.playMode === 'orderPlay') {
        ...
        this.setCurrentSong(
          this.playList[parseInt(Math.random() * this.playList.length)]
        )
        setTimeout(() => {
          this.audio.play()
        }, 20)
    ...
    timeupdate(e) {
      if (!this.isSetProgress) {
        ...
        this.progress =
          (100 * e.target.currentTime) /
          (this.currentSong.transitionTime / 1000)
      }
    },
    // 静音按钮点击事件
    mutedClick() {
