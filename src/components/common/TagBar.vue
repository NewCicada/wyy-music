    ...
    <div class="category-container">
      ...
      <ul>
        ...
        >
          <a
            href="javascript:;"
            :data-index="index"
            @click="switchHotTags($event.target.dataset.index, tag)"
            >{{ tag.name }}</a
          >
        </li>
      </ul>
      <div class="hot" v-if="videoPage">
        <span
          ...
          :class="{ 'hot-active': songSheetParams.order === 'hot' }"
          >热门</span
        >
        <span
          @click="chooseType('new')"
          :class="{ 'hot-active': songSheetParams.order === 'new' }"
          >最新</span
        >
      </div>
      <div class="video-all" v-else>
        <span @click="loadALlVideo" :class="{ active: mainTag === '全部' }"
          >全部</span
        >
      </div>
    </div>
